...
public class SceneController : MonoBehaviour
{
    ...
    private void PrepareField(AiDeck deck, List<int> hand_list, bool is_player_deck)
    {
        ...
        // Instantiate a card back for each list item and set as child to Player/AiDeck/DeckBase
    ...
    public void DirectlyPlaceCard(GameObject card_go, GameObject field_go, bool is_player_turn)
    {
        ...
        //Debug.Log("AiCard type is: " + card_type);
        //Debug.Log("AiCard row is: " + card_row);
        ...
        //Debug.Log("AiCard placed successfully! ");
        if (is_player_turn)
            StartCoroutine(EnemyTurn());
        else
            StartCoroutine(PlayerTurn());
    ...
    //-----------------------------------------Generate AiDeck, Exchange / Discard---------------------------------//
    // Returns a List of integers containing the card ids
    private List<int> GenerateDecklist(AiDeck deck)
    {
    ...
    private List<int> GetMedicList(List<int> my_list)
    {
        ...
        List<AiCard> card_list = GetCardsList(my_list);
        foreach (AiCard card in card_list)
        {
    ...
    // Get AiCard Stats from a card id
    ...
    public List<AiCard> GetCardsList(List<int> card_list)
    {
    ...
    {
        List<AiCard> my_cards = GetCardsList(card_list);
        foreach (AiCard card in my_cards)
        {
    ...
    {// For unit cards exclusively
        ...
        List<AiCard> cards_list = GetCardsList(units_list);
    ...
    private void Muster(CardStats card_stats, string row, bool is_player_turn)
    {
        ...
        Debug.Log("Name: " + name);
        List<AiCard> cards_list = GetCardsList(my_info.DeckList);
    ...
    private void RndMedic(bool is_player_turn)
    {
        ...
        if (units.Count > 0)
        {
            ...
            AiCard card = GetCardStats(units[r]);
    ...
    public void SetLeaderCard(int leader_id, bool is_player_deck)
    {
        ...
        AiCard my_card = GetCardStats(leader_id);
    ...
    // Initialize the AiDeck List on screen
    ...
    public void OptimizeAgile(bool is_player_turn)
    {
        ...
        List<AiCard> close_cards = GetCardsList(info.CloseList);
        ...
        List<AiCard> range_cards = GetCardsList(info.RangeList);
