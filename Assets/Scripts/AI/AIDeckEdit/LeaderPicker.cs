...
public class LeaderPicker : MonoBehaviour
{
    ...
    private void CreateLeaders(List<int> leaders_list)
    {
        for (int i = 0; i < leaders_list.Count; i++)
        {
            ...
            Debug.Log(Resources.Load<TextAsset>("Cards/EffectBox/Leader/" + leaders_list[i]).text);
            instantiatedLeader.transform.Find("LEffect/text").GetComponent<TextMeshProUGUI>().text =
                Resources.Load<TextAsset>("Cards/EffectBox/Leader/" + leaders_list[i]).text;
